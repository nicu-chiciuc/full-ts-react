...
const mockLoginResponse = {
  expiry: '2020-01-01T10:00:00.000Z',
  token: 'abcd123',
}
...
it('conditionally renders based on session', () => {
  ...
  expect(invalidSessionWrapper.find('div[className="App-login"]')).toHaveLength(
    1
  )
  ...
  expect(validSessionWrapper.find('div[className="App-private"]')).toHaveLength(
    1
  )
})
...
it('can handle login', done => {
  mock
    .onPost('/api/users/login', mockUserCredentials)
